...
def redaccion_view(request):
    if request.user.userprofile.rol not in [Roles.REDACCION, Roles.REDACTOR_IA, Roles.PRENSA, Roles.ADMIN]:
        return render(request, "403.html", status=403)
...
def informe_banda_crear_view(request):
    if request.user.userprofile.rol not in [Roles.REDACCION, Roles.REDACTOR_IA, Roles.ADMIN]:
        return render(request, "403.html", status=403)
...
def articulo_editar_view(request, id):
    ...
    if (
        request.user.userprofile.rol in [Roles.REDACCION, Roles.REDACTOR_IA]
...
def procesar_links_ia(request):
    ...
    for link_id in link_ids:
        ...
        link.estado = EstadoLink.APROBADO
    ...
    status_code = 200 if procesados or errores else 400
    return JsonResponse(
...
def informes_view(request):
    ...
    usuarios = User.objects.filter(userprofile__rol__in=[Roles.ADMIN, Roles.REDACCION, Roles.REDACTOR_IA, Roles.PRENSA])
...
def _obtener_estadisticas_redaccion(desde=None, hasta=None):
    usuarios = list(
        User.objects.filter(userprofile__rol__in=[Roles.REDACCION, Roles.REDACTOR_IA])
        .order_by("first_name", "last_name", "username")
    )
...
def consultar_articulo_view(request, id):
    ...
    if request.user.userprofile.rol not in [Roles.ADMIN, Roles.REDACCION, Roles.REDACTOR_IA, Roles.CLIENTE]:
...
@login_required
def consulta_bandas_view(request):
    """
    ...
    # Recoger los filtros del GET
    nombre_banda = request.GET.get("nombre_banda", "").strip()
    alias = request.GET.get("alias", "").strip()
    zonas_influencia = request.GET.get("zonas_influencia", "").strip()
    lider_nombre = request.GET.get("lider_nombre", "").strip()
    lider_alias = request.GET.get("lider_alias", "").strip()
    lider_dni = request.GET.get("lider_dni", "").strip()
    miembro_nombre = request.GET.get("miembro_nombre", "").strip()
    miembro_alias = request.GET.get("miembro_alias", "").strip()
    miembro_dni = request.GET.get("miembro_dni", "").strip()
    banda_aliada = request.GET.get("banda_aliada", "").strip()
    lider_banda_aliada = request.GET.get("lider_banda_aliada", "").strip()
    banda_rival = request.GET.get("banda_rival", "").strip()
    lider_banda_rival = request.GET.get("lider_banda_rival", "").strip()
    ...
    if zonas_influencia:
        bandas = bandas.filter(zonas_influencia__icontains=zonas_influencia)
    if lider_nombre:
        bandas = bandas.filter(lideres__nombre__icontains=lider_nombre)
    if lider_alias:
        bandas = bandas.filter(lideres__alias__nombre__icontains=lider_alias)
    if lider_dni:
        bandas = bandas.filter(lideres__documento__icontains=lider_dni)
    if miembro_nombre:
        bandas = bandas.filter(miembros__nombre__icontains=miembro_nombre)
    if miembro_alias:
        bandas = bandas.filter(miembros__alias__nombre__icontains=miembro_alias)
    if miembro_dni:
...
@login_required
def api_detalle_banda(request, id):
    """
    ...
    try:
        banda = get_object_or_404(
            BandaCriminal.objects.prefetch_related(
                "lideres__alias",
                "lideres__telefono",
                "miembros__alias", 
                "miembros__telefono",
        ...
        logger.error(f"Error al obtener detalle de banda ID {id}: {e}")
        return JsonResponse({"status": "error", "msg": "Error interno al procesar la banda."}, status=500)

    # Función auxiliar para serializar personas
    def serializar_persona(persona):
        return {
            "id": persona.id,
            "nombre": f"{persona.nombre} {persona.apellido}".strip(),
            "documento": persona.documento or "",
            "alias": [alias.nombre for alias in persona.alias.all()],
            "telefonos": [telefono.numero for telefono in persona.telefono.all()],
            "rol": persona.get_rol_display() if persona.rol else "",
            "situacion": persona.get_situacion_display() if persona.situacion else "",
        }

    # Obtener líderes y miembros
    lideres = [serializar_persona(lider) for lider in banda.lideres.all()]
    miembros = [serializar_persona(miembro) for miembro in banda.miembros.all()]
    ...
    zonas_influencia_formateadas = []
    if hasattr(banda, 'zonas_influencia'):
        zonas_data = banda.zonas_influencia or []
        if isinstance(zonas_data, list) and zonas_data:
            if isinstance(zonas_data[0], dict):
                zonas_influencia_formateadas = zonas_data
            else:
                zonas_influencia_formateadas = [str(zona) for zona in zonas_data]
        else:
            zonas_influencia_formateadas = []

    # CORRECCIÓN: Preparar datos del líder principal para el frontend
    lider_principal_data = {}
    if lideres:
        primer_lider = lideres[0]
        lider_principal_data = {
            "nombre": primer_lider["nombre"],
            "alias": ", ".join(primer_lider["alias"]) if primer_lider["alias"] else "",
            "dni": primer_lider["documento"] or "",
            "descripcion": f"Rol: {primer_lider['rol']} - Situación: {primer_lider['situacion']}" if primer_lider['rol'] or primer_lider['situacion'] else "Sin información adicional"
        }
    else:
        lider_principal_data = {
            "nombre": "Sin líder asignado",
            "alias": "",
            "dni": "",
            "descripcion": "No se ha asignado un líder para esta banda"
        }
    ...
    data = {
        ...
        "lideres": lideres,
        "miembros": miembros,
        "num_miembros": banda.miembros.count(),
        "bandas_aliadas": [
            {
                "id": aliada.id,
        ...
        "actividades_delictivas": [
            {
                "tipo": actividad.get_categoria_display() if actividad.categoria else "Sin categoría",
                "descripcion": actividad.descripcion or "Sin descripción"
            }
            for actividad in banda.hechos_delictivos.all()
        ]
    }

    return JsonResponse({"status": "ok", "banda": data})


@login_required
def exportar_banda_pdf(request, banda_id):
    """
    Exportar banda criminal a PDF
    """
    if HTML is None:
    ...
    try:
        ...
        logger.error(f"Error al obtener banda para PDF ID {banda_id}: {e}")
    ...
    context = {
        "banda": banda,
        "lideres": banda.lideres.all(),
        "miembros": banda.miembros.all(),
        "bandas_aliadas": banda.bandas_aliadas.all(),
        "bandas_rivales": banda.bandas_rivales.all(),
        "hechos_delictivos": banda.hechos_delictivos.all(),
        "fecha_generacion": localtime(now()).strftime("%d/%m/%Y %H:%M")
    }

    html_string = render_to_string("banda_pdf.html", context)
    
    try:
        pdf = HTML(string=html_string, base_url=request.build_absolute_uri()).write_pdf()
    except Exception as e:
        logger.error(f"Error al generar PDF de banda ID {banda_id}: {e}")
        messages.error(request, "Error al generar el PDF de la banda.")
        return redirect('consulta_bandas')
